...
def alert(message):
    global alert_playing, alert_message, alert_start_time
    with alert_lock:
        if alert_playing:
            return
        alert_playing = True
    ...
    def play_sound():
        ...
        song = AudioSegment.from_mp3("beep-warning-6387.mp3")  
        play(song[:1000])
        with alert_lock:
            alert_playing = False
...
# Open video capture
video_source = "test.mp4" # Change to "0" for webcam
cap = cv2.VideoCapture(video_source if video_source != "0" else 0)

# Set up video writer
fourcc = cv2.VideoWriter_fourcc(*'mp4v')
out = cv2.VideoWriter('sp.mp4', fourcc, 20.0, (int(cap.get(3)), int(cap.get(4))))
